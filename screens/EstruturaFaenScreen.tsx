...
import { View, Text, StyleSheet, Image, ScrollView, TouchableOpacity, ImageBackground, Alert, Linking, TextInput, KeyboardAvoidingView, Platform } from 'react-native';
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        const sectionNamesInOrder = ['História', 'Informação Importante', 'Equipe', 'Grupos_Importantes', 'Documentos_Importantes'];
        ...
        const processedSections = sectionNamesInOrder.map(name => {
          ...
          if (name === 'Equipe' || name === 'Grupos_Importantes' || name === 'Documentos_Importantes') {
          ...
          <>{/* Fragment Principal */}
            ...
            {sections.map((section, index) => (<View key={index}>{/* Container da Seção */}
              <Text style={styles.subtitle}>
                {section.section === 'Grupos_Importantes'
                  ? 'Grupos Importantes'
                  : section.section === 'Documentos_Importantes'
              ...
              <View style={styles.infoCard}>{/* Card da Seção */}
                ...
                {editing === section.section ? (<>{/* Início Modo Edição */}
                  ...
                  {/* Form Edição Documentos */}
                  {section.section === 'Documentos_Importantes' && (<>
                    <Text style={styles.editLabel}>Editar Documentos Importantes</Text>
                    {/* Lista de documentos */}
                    {Array.isArray(editingContent?.items) && editingContent.items.map((documento: string, idx: number) => (
                      <View key={`edit-doc-${idx}`} style={styles.editableItemContainer}>
                        <TextInput
                          style={styles.textInput}
                          value={documento}
                          placeholder="Nome do Documento"
                          onChangeText={(text) => setEditingContent((prev: any) => ({
                            ...prev,
                            items: prev.items.map((d: any, i: number) => i === idx ? text : d)
                          }))}
                        />
                        <TouchableOpacity
                          style={styles.deleteButton}
                          onPress={() => setEditingContent((prev: any) => ({
                            ...prev,
                    ...
                    <TextInput
                      ...
                      placeholder="Link dos Documentos"
                      keyboardType="url"
                      onChangeText={(text) => setEditingContent((prev: any) => ({ ...prev, link: text }))}
                    />
                    <TouchableOpacity
                  ...
                  {/* Form Edição Texto */}
                  {section.section !== 'Equipe' && section.section !== 'Grupos_Importantes' && section.section !== 'Documentos_Importantes' && (<>
                    <Text style={styles.editLabel}>Editar {section.section}</Text>
                  ...
                  <View>{/* Container Visualização + Lápis */}
                    {Array.isArray(section.content?.items) ? (<>{/* É Array de Itens */}
                      ...
                      {section.section === 'Documentos_Importantes' && (
                        <>
                          {/* Lista os documentos como tópicos */}
                          {Array.isArray(section.content.items) && section.content.items.map((documento: string, idx: number) => (
                            <View key={`view-doc-${idx}`} style={styles.documentItem}>
                              <Icon name="document-text" size={20} color="#00008B" style={styles.documentIcon} />
                          ...
                          {section.content?.link && (
                            <TouchableOpacity
                              ...
                              onPress={() => openLink(section.content.link)}
                            >
                              <Icon name="folder-open" size={24} color="#FF6B35" style={styles.documentLinkIcon} />
                              <Text style={styles.documentLinkText}>Acessar Documentos</Text>
                            </TouchableOpacity>
                          )}
                        </>
                      )}
                      {section.section !== 'Equipe' && section.section !== 'Grupos_Importantes' && section.section !== 'Documentos_Importantes' && section.content.items.map((item: any, idx: number) => (<Text key={`gen-item-${idx}`} style={styles.infoText}>{item.text || `Item ${idx + 1} sem texto`}</Text>))}
